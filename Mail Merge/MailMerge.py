...
def combine_files(src_directory):
    ...
    for f in filelist:
        for sheet in pandas.ExcelFile(f).sheet_names:
            df = pandas.read_excel(f,sheet)
...
def reorder_columns(dataframe, seq):
    ...
    for x in cols:
        if x not in dataframe.columns: #If column from seq is not in df,
            dataframe[x]= numpy.nan   #create a new column filled with Nan
...
def main():
    ...
    usadd = pandas.DataFrame(columns=ls)
    wrongaddress = pandas.DataFrame(columns=ls)
    global output_df
